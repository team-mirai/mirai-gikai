...
export const DEFAULT_QUESTIONS_TEMPLATE: readonly DefaultQuestionTemplateEntry[] =
  [
    ...
    },
    {
      ...
      follow_up_guide:
        "「いいえ」の場合は、見落とされそうな具体例（例: 属性や立場の違い（個人・事業者・組織など）、規模や条件の違い（小規模／大規模、地域差、就労形態など）、既存の仕組みとの関係、対象から漏れそうなケースなど）を引き出す。「わからない」の場合は、無理せず次の質問に進む。具体的なキーワードを含む回答を得られた場合は深掘りをやめて次の質問に行く。なるべく一度の質問で回答者から具体的な回答を得るようにこころがけ、長々と質問を続けない。回答者とのやりとりは最大5往復までにとどめる。",
      quick_replies: [
