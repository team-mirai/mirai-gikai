...
### 3. role（立場・属性）
- **ログ内でユーザー本人が申告した立場**（「どういう立場で関わっていますか」等への回答）を最優先の根拠とすること。経歴や背景の話題から推測で上書きしない
...
- ログ内の本人発言のみを根拠にする。過去の経歴は「元〜」「〜した経験がある」のように、現在の立場と誤読されない表現にする

### 5. role_title（立場の短縮タイトル）
- role_descriptionを10文字以内で端的に表現したタイトル
- 例：「物流業者」「主婦」「教師」「IT企業経営者」など
- 本人申告の立場を反映し、過去の経歴を現在の職業のように表記しない（過去の職歴なら「元〜」を付ける）
- **重要**: 必ず10文字以内にすること
