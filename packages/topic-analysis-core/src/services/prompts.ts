...
- 説明は対象意見群の要点を **箇条書き（最大3行）** で示す。平文の段落にせず、1要点=1行に分ける。
  - 各行は **改行（\\n）で区切る**。「・」「-」「*」等の箇条書き記号や番号は付けない（記号は表示側で付与する）。
  - 各行は1文で簡潔に（1行あたり40字程度を目安）、必ず述語で締める。**体言止め・名詞止め（「〜という期待。」「〜という懸念。」「〜への対応。」等）で終わらせない**（例:「〜を懸念する声がある。」「〜が指摘されている。」「〜を求める意見がある。」）。
  - 余計な装飾や引用を盛り込まず、論点の核を端的に述べる。
  - 記述例（3行・改行区切り。各行の文だけを出力し記号は付けない）:
